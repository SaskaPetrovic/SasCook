...
class recipeController extends Controller
{
     ...
     public function allRecipies(Request $request)
     {
         ...
         $allRecipies = t_recette::select(
             ...
             t_ingredient::raw('GROUP_CONCAT(DISTINCT CONCAT_WS(" ", t_utiliser.utiQuantite, 
                     t_ingredient.ingUniteDeMesure, t_ingredient.ingNom) SEPARATOR ", ") as ingredients')
         ...
         $ingredients = t_ingredient::select(
             't_ingredient.idIngredient',
             't_ingredient.ingNom',);
         ...
         //chatGpt m'a aidé pour afficher que les ingredients et les catégories utilisés
         ...
         $usedCategorie = t_appartenir::distinct('idCategorie')->pluck('fkCategorie');
         $categories = t_categorie::whereIn('idCategorie', $usedCategorie)->get();
         ...
         if ($query) {
             ...
             $searchRecipe = t_recette::select(
                 ...
                 ->leftJoin('t_appartenir', 't_recette.idRecette', '=', 't_appartenir.fkRecette')
                 ->leftJoin('t_categorie', 't_appartenir.fkCategorie', '=', 't_categorie.idCategorie')
                 // recherche de titre de recette contenant la requête
                 ->where('t_recette.recTitre', 'like', "%$query%")
                 ->orderBy('t_recette.created_at', 'desc')
                 ->groupBy('t_recette.idRecette')
                 ->get();
     ...
     public function descriptionRecipies($id, Request $request)
     {
         ...
         $infoRecipies =  t_recette::select(
             ...
             't_recette.recTitre',
             't_recette.recTemps',
             't_recette.recPreparation',
             't_recette.recNbDePersonne',
             't_recette.recImageLien',
             't_recette.created_at',
             t_categorie::raw('GROUP_CONCAT(DISTINCT t_categorie.catNom SEPARATOR ", ") as categorie'),
             t_ingredient::raw('GROUP_CONCAT(DISTINCT CONCAT_WS(" ", t_utiliser.utiQuantite,
         ...
         if ($request->has('servings')) {
             ...
             // Pas de soumission de mise à jour de quantités donc cela utilise les quantités- 
             // -par défaut dans la base de donnée
             $ingredients = t_utiliser::select('utiQuantite', 'ingUniteDeMesure', 'ingNom')
                 ->join('t_ingredient', 't_utiliser.fkIngredient', '=', 't_ingredient.idIngredient')
                 ->where('t_utiliser.fkRecette', $id)
                 ->get();
