    ...
    <section class="text-gray-600 body-font">
        <div class="container mx-auto flex px-0 py-16 md:flex-row flex-col items-center">
            <div class="lg:max-w-lg lg:w-full md:w-1/2 w-5/6 mb-10 md:mb-0">
                <img class="object-cover object-center rounded" alt="recette" id="AccImgSize" src='{{"/img/".$lastRecipe -> recImageLien }}'>
            </div>
            <div class="lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center text-center">
                <h2 class="title-font mb-4 font-medium text-gray-500">{{ $lastRecipe->categorie }}</h2>
                <h1 class="title-font sm:text-4xl text-3xl mb-4 font-medium text-gray-800">{{ $lastRecipe-> recTitre}}</h1>
                <p class="mb-8 title-font font-medium text-gray-500">Ajoutée le : {{ date("Y-m-d", strtotime($lastRecipe->created_at))}} à {{ date("H:i:s", strtotime($lastRecipe->created_at))}}</p><!--stack overflow-->
                <div class="flex justify-center">
                    <a href="{{ url('/description',['id' => $lastRecipe->idRecette]) }}" class=" inline-flex items-center mt-3 text-indigo-400">En savoir plus
